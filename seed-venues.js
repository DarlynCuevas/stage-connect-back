...
const venuesData = [
  ...
  },
  {
    ...
    gallery: [
      ...
      'https://images.unsplash.com/photo-1520637836862-4d197d17c53a?w=400'
    ]
  },
  {
...
async function seedVenues() {
  try {
    ...
    const venueRepository = AppDataSource.getRepository(VenueProfile);
    ...
    for (const venueData of venuesData) {
      ...
      const user = userRepository.create({
        ...
        password: '$2b$10$rQ8K3tOjG5Pf2zG4hW1Lb.vX2mJ9nK5oP6qR7sT8uV9wX0yZ1aB2c', // password: "123456"
        role: 'Local',
        city: venueData.city,
        province: venueData.province,
        bio: venueData.bio,
        avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${venueData.name.toLowerCase().replace(/\s/g, '')}`,
        phone: venueData.phone,
      });

      const savedUser = await userRepository.save(user);

      // Crear el perfil de venue
      const venueProfile = venueRepository.create({
      ...
      await venueRepository.save(venueProfile);
      createdCount++;

      console.log(`✅ Creado: ${venueData.name} en ${venueData.city}`);
    }

    console.log(`\n🎉 Proceso completado: ${createdCount} locales creados exitosamente`);
    
    // Mostrar estadísticas
    const totalVenues = await userRepository.count({ where: { role: 'Local' } });
    console.log(`📊 Total de locales en la base de datos: ${totalVenues}`);
